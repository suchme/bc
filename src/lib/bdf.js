Bdf = new function(){
	...
	var Bdf = function(){
		...
		this.bdfChars = new Object()
	}

	this.load=function(url,bdf,callback){
		if(!bdf){
			bdf = new Bdf();
		}
	...
	var code_lf='\n'.charCodeAt(0)
	,code_unknown='X'.charCodeAt(0)
	this.renderBdfC= function(bdf,code){
		var width,height
		,j,jmax
		,fontheight=bdf.size[0]
		,k,kmax
		,idx

		bdfChar = bdf.bdfChars[code]
		if(!bdfChar) bdfChar = bdf.bdfChars[code_unknown]

		jmax=bdfChar.bitmap.length
		kmax=bdfChar.dwidth

		imagedata= context.createImageData(kmax,jmax)
		data=imagedata.data
		width4=kmax*4
		...
		this.textwidth=kmax
		this.textheight=jmax
		return canvas
	}
	this.renderBdfCV= function(bdf,code){
		var width,height
		,j,jmax
		,fontheight=bdf.size[0]
		,k,kmax
		...
		if(!(code&0x7fffff00)
			|| /[〈〉《》「」『』【】〔〕ー＝～]/.test(String.fromCharCode(code))){

			imagedata= context.createImageData(bdf.size[0],bdfChar.dwidth)
			data=imagedata.data
			width4=jmax*4
			for(j=0;j<jmax;j++){
				bits=bdfChar.bitmap[j]
		...
		}else if( /[。、]/.test(String.fromCharCode(code))){
			imagedata= context.createImageData(bdf.size[0],bdfChar.dwidth)
			data=imagedata.data
			bits=bdfChar.bitmap[j]
			...
			imagedata= context.createImageData(bdfChar.dwidth,bdf.size[0])
			data=imagedata.data
			bits=bdfChar.bitmap[j]
			width4=imagedata.width*4
			for(j=0;j<jmax;j++){
		...
		context.putImageData(imagedata,0,0)
		this.textwidth=imagedata.width
		this.textheight=imagedata.height
		return canvas
	}
	this.render= function(text,bdf,flg){
		var width,height
		,i,imax=text.length
		,j,jmax
		,fontheight=bdf.size[0]
		...
		width=height=512;
		var imagedata= Util.ctx.createImageData(width,height)
		...
		for(i=0;i<imax;i++){
			...
			if(flg)x+=fontheight
			else x+=bdfChar.dwidth//kmax
		}
		return imagedata
	}
	this.renderAscii=function(bdf){
		var str="";
		for(i=32;i<128;i++){
	...
	this.renderBdfV= function(bdf,text){
		var width,height
		,i,imax=text.length
		,j,jmax
		,k,kmax
...
,0x2D6E:0x337D
,0x2D6F:0x337C
//,0x3-0x2D70		# <reserved>	Windows:0x U+0x2252
//,0x3-0x2D71		# <reserved>	Windows:0x U+0x2261
//,0x3-0x2D72		# <reserved>	Windows:0x U+0x222B
,0x2D73:0x222E
//,0x3-0x2D74		# <reserved>	Windows:0x U+0x2211
//,0x3-0x2D75		# <reserved>	Windows:0x U+0x221A
//,0x3-0x2D76		# <reserved>	Windows:0x U+0x22A5
//,0x3-0x2D77		# <reserved>	Windows:0x U+0x2220
,0x2D78:0x221F
,0x2D79:0x22BF
//,0x3-0x2D7A		# <reserved>	Windows:0x U+0x2235
//,0x3-0x2D7B		# <reserved>	Windows:0x U+0x2229
//,0x3-0x2D7C		# <reserved>	Windows:0x U+0x222A
,0x2D7D:0x2756
,0x2D7E:0x261E
,0x2E21:0x4FF1
,0x2E22:0x2000B
